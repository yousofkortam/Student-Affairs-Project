...
<form method="post" action="{{url('/admin/add-new-course')}}" class="form">
    ...
    <div class="flex">
        <label>
            <input name="course_name" required="" type="text" class="input">
            <span>Name</span>
        </label>

        <label>
            <input name="course_code" required="" placeholder="" type="text" class="input">
            <span>Code</span>
        </label>
    </div>
    <label>
    ...
    <label>
        <select class="form-select input" type="number" name="professor_id">
            @foreach ($data['profs'] as $prof)
                <option value="{{$prof->id}}">Dr.{{$prof->user->first_name}} {{$prof->user->last_name}}</option>
            @endforeach
          </select>
        <span>Professor</span>
    </label>
